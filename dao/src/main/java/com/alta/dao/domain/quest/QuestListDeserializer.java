...
public class QuestListDeserializer implements JsonDeserializer<List<QuestListItemModel>> {
    ...
    private final List<QuestNameMatcher> matchers;
    ...
    private QuestListItemModel parseListItem(JsonObject jsonQuestListItem) {
        try {
            return new QuestListItemModel(
                    jsonQuestListItem.get(UUID_FIELD_NAME).getAsString(),
                    jsonQuestListItem.get(NAME_FIELD_NAME).getAsString(),
                    jsonQuestListItem.get(PATH_TO_DESCRIPTOR_FIELD_NAME).getAsString()
            );
        } catch (Exception e) {
